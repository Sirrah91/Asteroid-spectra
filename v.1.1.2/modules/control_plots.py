...
def plot_error_range_plots(error_type: str = 'RMSE_all_nooutliers', applied_function=np.mean,
                           line_plot_or_matrix: str = 'both') -> None:
    def line_plot(ax_line):
        num_colors = len(wvl) - 1
        cm = plt.get_cmap('gist_rainbow')

        ax_line.set_prop_cycle(color=[cm(1. * i / num_colors) for i in range(num_colors)])

        for j in range(len(wvl) - 1):
            x = [int(ranges[i].split()[-1][:-1]) for i in range(len(ranges)) if
                 int(ranges[i].split()[0][1:-1]) == wvl[j]]
            y = [rmse[i] for i in range(len(rmse)) if int(ranges[i].split()[0][1:-1]) == wvl[j]]

            ax_line.plot(x, y, '--o', label='From ' + str(wvl[j]) + ' nm')

        ax_line.set_xticks(wvl[1:])
        ax_line.set_xlabel('To wavelength [nm]')
        ax_line.set_ylabel(error_label)
        ax_line.tick_params(labelbottom=True, labeltop=False, labelleft=True, labelright=False,
                            bottom=True, top=False, left=True, right=False)

        ax_line.legend(bbox_to_anchor=(0, 1.02, 1, 0.2), loc="lower left", mode="expand", borderaxespad=0, ncol=4)

    def matrix_plot(ax_matrix):
        im = ax_matrix.imshow(res_mat, aspect="auto", cmap='jet_r',
                              vmin=np.nanmin(res_mat), vmax=np.nanmin((15, np.nanmax(res_mat))))
    ...
    # load data
    filename = project_dir + '/range_test_data/data_spacing_10.dat'
    ...
    pos_under = [m.start() for m in re.finditer('_', error_type)]
    error_label = error_type[:pos_under[0]]

    if len(pos_under) > 1:  # OPX or CPX
        error_label = error_label + ' (' + error_type[pos_under[0] + 1:pos_under[1]] + ', ' + error_type[pos_under[1] +
                                                                                                         1:] + ')'
    else:  # OL or modal
        error_label = error_label + ' (' + error_type[pos_under[0] + 1:] + ')'

    wvl = np.arange(450, 2451, 100)
    ranges, rmse = combine_same_range_models(indices, ranges_all, rmse_all, applied_function)
...
def plot_conv_kernels(model: Functional = None):
    if model is None:
...
def plot_surface_spectra(y_pred: np.ndarray, filename: str, what_type: str) -> None:
    ...
    n, _ = y_pred.shape
    sum_of_predictions = np.sum(y_pred, axis=0) / n * 100
