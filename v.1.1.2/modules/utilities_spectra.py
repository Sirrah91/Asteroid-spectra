...
def combine_same_range_models(indices, ranges_all, what_rmse_all, applied_function):
    ...
    for ind, unique_index in enumerate(np.unique(indices)):
        ranges[ind] = ranges_all[np.where(unique_index == indices)[0]][0]
        what_rmse[ind] = applied_function(what_rmse_all[np.where(unique_index == indices)[0]])

    return ranges, what_rmse
