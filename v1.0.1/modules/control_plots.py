...
from modules.CD_parameters_RELAB import path_relab, path_tuomas
...
def plot_scatter_plots(y_true: np.ndarray, y_pred: np.ndarray) -> None:
    ...
    if num_minerals > 1:
        ...
        fig_name = 'scatter_plot_modal.eps'
    ...
    for i in range(len(subtypes)):
        ...
        fig_name = "".join(('scatter_plot_', minerals_used[i], '.eps'))
        fig.savefig("".join((outdir, '/', fig_name)), format='eps', bbox_inches='tight', pad_inches=0)
        plt.close(fig)
...
def plot_Fa_vs_Fs_v2(y_true: np.ndarray, y_pred: np.ndarray, model: int = 1) -> None:
    print('Plot Fa vs Fs')
    ...
    for i in range(2):
        ...
        ax[i].set_xlabel('Fa', fontsize=font_size)
        if i == 0:
            ax[i].set_ylabel('Fs', fontsize=font_size)
        ...
        if model == 1:
            ...
            ax[i].set_xticks(np.arange(limx1, limx2 + 0.1, 20))
            ax[i].set_yticks(np.arange(limy1, limy2 + 0.1, 20))
        ...
        ax[i].legend(fontsize=font_size/2)

    plt.draw()
    plt.tight_layout()

    fig_name = 'Fa_vs_Fs_v2.eps'
    fig.savefig("".join((outdir, '/', fig_name)), format='eps', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
...
def plot_Fa_vs_Fs_Tuomas(y_pred_or_y_true: np.ndarray) -> None:
    ...
    filename = path_tuomas + 'AP_spectra-norm-denoised.dat'
    ...
    inds_S = np.array(['S' in OC_type for OC_type in types])
    inds_Q = np.array(['Q' in OC_type for OC_type in types])
    inds_A = np.array(['A' in OC_type for OC_type in types])
    inds_R = np.array(['R' in OC_type for OC_type in types])  # not present in dataset
    inds_O = np.array(['O' in OC_type for OC_type in types])  # not present in dataset
    inds_V = np.array(['V' in OC_type for OC_type in types])
    ...
    ax.set_xlabel('Fa', fontsize=font_size)
    ax.set_ylabel('Fs', fontsize=font_size)
    ax.tick_params(axis='both', labelsize=font_size)
    ax.axis('square')
    ...
    # ax.legend(lns, labs, loc='upper right', fontsize=font_size)

    plt.draw()
    plt.tight_layout()

    fig_name = 'Fa_vs_Fs_tuomas.eps'
    fig.savefig("".join((outdir, '/', fig_name)), format='eps', bbox_inches='tight', pad_inches=0)
    plt.close(fig)
