...
def pipeline(n_models: int = 1) -> Union[np.ndarray, None]:
    # Computes a trimmed mean of n_models
    if train_new_model or tune_hyperparameters:
        ...
        if tune_hyperparameters:
            ...
            predictions, accuracy = evaluate_test_data(model_names, x_test, y_test, x_val, y_val, x_train, y_train)
    else:
        # Name of the model in ./Models/chemical/
        model_names = ['20220107150855_CNN.h5']

        # Name of the new data in ./Datasets/
...
from modules.NN_data import load_data, split_data_proportional
from modules.NN_evaluate import evaluate_test_data, evaluate
from modules.tables import mean_asteroid_type, accuracy_table, quantile_table
from modules.NN_config import num_labels, use_minerals_all
from modules.control_plots import plot_ast_PC1_PC2
...
filename_data = 'Tomas_Chelyabinsk-denoised-norm-nolabel.dat'
y_pred = evaluate(model_names, filename_data)
"""
