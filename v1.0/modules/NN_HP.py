...
def gimme_hyperparameters(composition_or_taxonomy: str, grid_option: Union[str, None]) -> Dict:
    ...
    if "composition" in composition_or_taxonomy:
        if grid_option is None:
            ...
            raise ValueError('"grid_option" must be None or contain "Itokawa" or "Eros".')
        ...
        if grid_option is None:
            p = {
                ...
                'n_nodes': [12, 20, 4],  # Number of nodes in hidden layers / number of filters for CNN
                'kern_size': 3,  # Width of the kernel (only if CNN)
                'dropout_input_hidden': 0.0,  # Dropout
                'dropout_hidden_hidden': 0.0,  # Dropout
                'dropout_hidden_output': 0.2,  # Dropout
                'L1_trade_off': 0.0033,  # L1 trade-off parameter
                'input_activation': 'relu',  # Activation function of input layer
