...
def evaluate_test_data(model_names: List[str], x_test: np.ndarray, y_test: np.ndarray,
    ...
    if show_result_plot:
        plot_scatter_plots(y_test, predictions, quiet=quiet, suf=gimme_suf(num_labels))
        plot_error_evaluation(y_test, predictions, quiet=quiet, suf=gimme_suf(num_labels))
