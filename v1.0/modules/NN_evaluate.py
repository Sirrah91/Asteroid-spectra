...
custom_objects = {loss_name: loss, output_activation_name: output_activation,
                  rmse_name: rmse, quantile_name: quantile, mae_name: mae, Lp_norm_name: Lp_norm,
...
def evaluate_test_data(model_names: List[str], x_test: np.ndarray, y_test: np.ndarray,
                       ...
                       x_train: np.ndarray = None, y_train: np.ndarray = None) -> Tuple[np.ndarray, ...]:
    ...
    if show_control_plot:
        if y_val is not None:
            plot_scatter_plots(y_val, predictions_val, type='_val')
        if y_train is not None:
            plot_scatter_plots(y_train, predictions_train, type='_train')
