...
def pen_OPX_v1(y_true: EagerTensor, y_pred: EagerTensor) -> EagerTensor:
    ...
    dist = relu(z_pred[:, -1] - OPX_Wo_limit) * beta
...
def pen_CPX_v1(y_true: EagerTensor, y_pred: EagerTensor) -> EagerTensor:
    ...
    dist = relu(z_pred[:, -1] - CPX_Wo_limit) * beta
...
def pen_PLG_v1(y_true: EagerTensor, y_pred: EagerTensor) -> EagerTensor:
    ...
    dist1 = relu(z_pred[:, use_plg_pen_idx[0]] - PLG_An_limit)
    dist2 = relu(PLG_Ab_limit - z_pred[:, use_plg_pen_idx[1]])
    dist3 = relu(z_pred[:, use_plg_pen_idx[2]] - PLG_Or_limit)
