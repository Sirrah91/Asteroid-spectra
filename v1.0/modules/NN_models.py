...
# metrics = [rmse, median, mae, Lp_norm, r2, sam]
metrics = [rmse]


def FC_model_old(x_train: np.ndarray, params: Dict) -> Sequential:
    model = Sequential()
    # Adding the input layer to the model
    model.add(Dropout(params['dropout_input_hidden'], input_shape=(x_train.shape[1],)))

    # Adding the hidden layers
    for i in range(params['n_layers']):  # last
        ...
        model.add(Dropout(params['dropout_hidden_hidden']))

    # Number of nodes in output layer
    model.add(Dense(num_labels, activation=output_activation))
...
def CNN_model_old(x_train: np.ndarray, params: Dict) -> Sequential:
    # input_shape = (params['batch_size'], np.shape(x_train)[1], 1)
    input_shape = (np.shape(x_train)[1], 1)

    model = Sequential()
    # Adding the input layer to the model
    model.add(Dropout(params['dropout_input_hidden'], input_shape=input_shape))
