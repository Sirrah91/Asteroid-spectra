...
def train(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray, params: Dict) -> Union[
    ...
    if params['tuning']:
        for_tuning, verbose = True, 0
    else:
        for_tuning, verbose = False, verb
    ...
    model_name = "".join((dt_string, '_', params['model_type'], '_', model_name_suffix, '.h5'))

    filename = "".join((project_dir, '/Models/', model_dir, '/', model_name))
    check_dir(filename)
    model.save(filename)
    ...
    if not quiet:
        ...
        acc = my_rmse(num_minerals, all_to_one=False)(y_train, model.predict(x_train)).numpy()
        print_accuracy(acc, 'train')

        if val_portion > 0:
            # Evaluate the validation data
            acc = my_rmse(num_minerals, all_to_one=False)(y_val, model.predict(x_val)).numpy()
...
def tune_hp_tuner(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
    ...
    now = datetime.now()
    dt_string = now.strftime("%Y%m%d%H%M%S")
    project_name = dt_string + '_chemical_' + model_name_suffix
    ...
    with open(project_dir + directory + '/' + project_name + '.csv', 'w') as f:
    ...
    if interpolate_to is None:
        ...
        model_name = "".join((dt_string, '_', interpolate_to, '_', model_name_suffix, '.h5'))
    filename = "".join((project_dir, '/Models/', model_dir, '/', model_name))
    check_dir(filename)
    ...
    if plot_correlation_matrix:
        ...
        for model_type in ['MLP', 'CNN']:
            ...
            if np.sum(where_models) > 1:
                ...
                plot_corr_matrix(keys, data, '_Tuner_' + method + '_' + model_type)
