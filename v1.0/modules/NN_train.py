...
def train(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray, params: Dict) -> Union[
    ...
    if val_portion > 0:
        ...
        early_stopping_monitor = EarlyStopping(monitor=acc_name, mode='min', patience=params['num_epochs'] * coef,
...
def tune_hp(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray) -> None:
    ...
    best = data_all.sort_values(acc_name, ascending=True).iloc[0][-len(p_for_tuning):]
    pprint(best)
