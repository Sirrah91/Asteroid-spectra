...
def collect_spectra(xq: np.ndarray, fnames: np.ndarray) -> np.ndarray:
    ...
    for i in range(N):
        ...
        if denoise:
            ...
            fun = interp1d(xq, v_final, kind='cubic')  # v_final differs from v
        else:
            v_final = v_interp
...
def save_data(final_name: str, spectra: np.ndarray, numbers: np.ndarray = None, meta: np.ndarray = None,
    ...
    if mixtures:
        ...
        if not Tuomas_data:
            ...
            filename = "".join((path_tuomas, final_name, '_nolabel.dat'))
    ...
    if numbers is not None:
        ...
        if not Tuomas_data:
            ...
            spectra = np.hstack((numbers, spectra))  # he stored labels in the first coordinate
...
def resample_Tuomas_data() -> None:
    ...
    if denoise:
        ...
        spectra = [np.convolve(np.squeeze(spectra[i]), kernel, 'same') / correction for i in
                   range(np.shape(spectra)[0])]
        fun = interp1d(x_new, spectra, kind='cubic')  # v_final differs from v

    if normalise:
        v_norm = fun(normalised_at)
    else:
        v_norm = np.ones(np.shape(fun(normalised_at)))

    spectra_final = np.transpose(np.divide(np.transpose(spectra), v_norm))

    save_data(final_name, spectra_final, numbers=labels, Tuomas_data=True)


def resample_AP_raw_data() -> None:

    # this function interpolate AP data to the given grid

    # new grid
    x_new = np.arange(450, 2450.1, 5)
    # final name
    final_name = 'AP_spectra'

    # load the data
    tuomas_data = 'MyVISNIR-final-sampled-collection.dat'
    data_file = "".join((project_dir, '/Datasets/Tuomas/', tuomas_data))
    data = pd.read_csv(data_file, sep='\t', header=None).to_numpy()

    # old grid
    x_old = data[0, 2:].astype(np.float32) * 1000  # nm

    # select spectra
    spectra = data[1:, 2:].astype(np.float32)
