...
from modules.NN_data import load_data, remove_nans, split_data
from modules.NN_losses_metrics_activations import my_rmse, my_r2, my_sam, my_quantile
from modules.NN_config import *
from modules.CD_parameters import path_tuomas, path_relab
...
def mean_asteroid_type(y_pred: np.ndarray) -> None:
    ...
    R = np.round(np.mean(predictions[inds_R, :], axis=0), 1)
    O = np.round(np.mean(predictions[inds_O, :], axis=0), 1)
    ...
    sigma_R = np.round(np.std(predictions[inds_R, :], axis=0, ddof=1), 1)
    sigma_O = np.round(np.std(predictions[inds_O, :], axis=0, ddof=1), 1)
    ...
    tmp = ['{:.1f}'.format(k1) + '+-' + '{:.1f}'.format(k2) for k1, k2 in zip(R, sigma_R)]
    print('R:', '[' + ', '.join(line for line in tmp) + ']')

    tmp = ['{:.1f}'.format(k1) + '+-' + '{:.1f}'.format(k2) for k1, k2 in zip(O, sigma_O)]
    print('O:', '[' + ', '.join(line for line in tmp) + ']')
...
def how_many_data(y_data: np.ndarray) -> None:
    ...
    rows = np.array([np.array([np.sort(np.round(y_data[result == res, i]*100, 1))[[0, -1]] for res in [4, 2, 1, 6, 5, 3, 7]]).ravel() for i in [0, 1, 2, 3, 5, 7, 8, 9]])
    print(rows)
    ...
    rows = np.array([np.array([np.sort(np.round(y_train[result == res, i]*100, 1))[[0, -1]] for res in [4, 2, 1, 6, 5, 3, 7]]).ravel() for i in [0, 1, 2, 3, 4, 6, 8, 9, 10]])
    print(rows)
    '''
...
def A_type_properties(y_pred: np.ndarray) -> None:
    ...
    filename_meta = path_tuomas + 'MyVISNIR-final-sampled-collection.dat'
    meta = pd.read_csv(filename_meta, sep='\t', header=None).to_numpy()  # to read the file

    ast_nums = meta[:, 0]
    nums = ['289', '354', '1951', '5261']
