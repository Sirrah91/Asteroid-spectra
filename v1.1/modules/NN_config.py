...
interpolate_to = None  # None for no change; other possibilities are "ASPECT", "Itokawa", "Eros"
...
p = {
    ...
    'optimizer': 'Adam',  # see return_optimizer and MyHyperModel.bulid in NN_models.py for options
...
p_for_tuning = {  # In talos format
    ...
    #
    # use [sorted, list, of, samples] -> after that Bayes and Random optimisations will work too
    #
    ...
    'dropout_hidden_hidden': [0.0, 0.2],  # Dropout rate
    'dropout_hidden_output': [0.2, 0.3, 0.4],  # Dropout rate
    'L1_trade_off': [0.0001, 0.01, 0.1, 1.0],  # L1 trade-off parameter
    'input_activation': ['relu', 'tanh', 'sigmoid'],  # Activation function of input layer
    'output_activation': ['sigmoid', 'softmax'],  # Activation function of output layer (relu, softmax, sigmoid)
    # Optimizer for the network
    'optimizer': ['Adam', 'SGD'],  # see return_optimizer and MyHyperModel.bulid in NN_models.py for options
    'learning_rate': [0.00005, 0.0005, 0.05],  # Learning rate
    'alpha': [0.01, 0.1, 1],  # Trade off between modal and chemical misfits
    'batch_size': [1, 4, 8, 16],  # Bath size
...
show_control_plot = False  # True for showing control plots, False for not
