...
def load_data(filename_data: str, clean_dataset: bool = False, keep_all_labels: bool = False,
              reinterpolation: bool = True) -> Tuple[np.ndarray, ...]:
...
def keep_pure_only(x_data: np.ndarray, y_data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if use_mix_of_the_pure_ones:  # mixtures if sum of unwanted == 0
        indices = np.where(np.sum(unwanted, axis=1) == 0)[0]
    else:  # pure if max of wanted == 1
        indices = np.where(np.max(wanted, axis=1) == 1)[0]

    return x_data[indices], y_data[indices]


def reinterpolate_data(x_data: np.ndarray, to_what_data: str = None) -> np.ndarray:
    ...
    if to_what_data is not None:
        ...
        if to_what_data == 'ASPECT':  # roughly ASPECT imager
            ...
            normalised_at_wvl = wvl_new[70]  # 1550
        elif to_what_data == 'Eros':  # Eros
            wvl_new_tmp = np.concatenate((np.arange(450, 550, 10), np.arange(560, 2451, 10)))

            wvl_new = wvl_new_tmp[[37, 39, 41, 43, 45, 47, 49, 52, 54, 56, 58, 60, 62, 65, 67, 69, 71, 73, 75, 77, 80,
                                   82, 109, 113, 117, 121, 126, 130, 134, 139, 143, 147, 156, 160, 164, 169, 173, 177,
                                   182, 186, 191, 199]]
            normalised_at_wvl = wvl_new[23]  # 1590
        else:
            raise ValueError('Unknown reolution of ' + to_what_data + '.')
...
def wt_to_vol_percent(y_data: np.ndarray) -> np.ndarray:
    ...
    densities = np.array([4.49, 3.27, 3.95, 3.20, 2.90, 3.95, 3.20, 2.90, 2.73, 2.62, 2.56])[flatten_list(subtypes_all)]
