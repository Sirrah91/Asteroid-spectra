...
def my_mse_all(num_minerals: int):
    def mse(y_true: EagerTensor, y_pred: EagerTensor) -> EagerTensor:
        # If numpy, convert to tensor
        if isinstance(y_true, np.ndarray):
            y_true, y_pred = K.constant(y_true), K.constant(y_pred)
        ...
        if num_minerals > 0:
            # Contribution of modal compositions
            w_true, w_pred = y_true[:, start:stop], y_pred[:, start:stop]
            res = K.square(w_true - w_pred)
            res = K.reshape(res, (tf.size(res), 1))
            ...
            for k in range(len(subtypes)):
                ...
                res_part = K.square(z_true - z_pred)
                res_part = K.reshape(res_part, (tf.size(res_part), 1))

                res = K.concatenate((res, res_part), axis=0)
        else:
            res = K.square(y_true - y_pred)

        res = tfnp.nanmean(res, axis=0)
...
def my_rmse_all(num_minerals: int):
    def rmse(y_true: EagerTensor, y_pred: EagerTensor) -> EagerTensor:
        return K.sqrt(my_mse_all(num_minerals)(y_true, y_pred))
