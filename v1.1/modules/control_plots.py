...
def plot_scatter_plots(y_true: np.ndarray, y_pred: np.ndarray, suf: str = '') -> None:
    print('Scatter plots')
...
def plot_surface_spectra(y_pred: np.ndarray, filename: str, what_type: str) -> None:
    ...
    if what_type == 'taxonomy':
        ...
        most_probable_classes = np.array([0, 3, 5, 9])  # olivine, Fa, Fs_opx, Wo_cpx (possible source of errors...)
        labels = ['olivine', 'Fa', 'Fs (OPX)', 'Wo (CPX)']
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray, suf: str = '') -> None:
    print('Confusion matrix')

    # upravit podle https://www.frontiersin.org/files/Articles/816268/fspas-09-816268-HTML-r1/image_m/fspas-09-816268-g002.jpg

    array = confusion_matrix(y_true.argmax(axis=1), y_pred.argmax(axis=1))
    '''
    sum_cols = np.sum(array, axis=0, keepdims=True)  # bude dole
    array = np.concatenate((array, sum_cols), axis=0)
    sum_rows = np.sum(array, axis=1, keepdims=True)  # bude nahore
    array = np.concatenate((array, sum_rows), axis=1)
    '''

    sum_rows = np.sum(array, axis=1, keepdims=True)  # bude nahore

    # tady nejak upravit dim
    dim = np.unique(y_true.argmax(axis=1))  # true values in the confusion matrix  # list(range(y_true.shape[1]))
    df_cm = pd.DataFrame(array / sum_rows, dim, dim)

    labels = np.array(list(classes.keys()))[dim]

    # labels = np.concatenate((labels, np.array(['\Sigma'])))

    fig = plt.figure("Confusion Matrix", figsize=(18, 15))
    sns.set(font_scale=1.4)  # label size
    ax = sns.heatmap(df_cm, annot=array, fmt="d", annot_kws={"size": 14}, cmap="Blues", cbar=False)
