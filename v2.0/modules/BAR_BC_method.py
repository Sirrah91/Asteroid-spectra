...
from modules.utilities import my_argmin, my_argmax
...
def calc_BAR_BC(wavelength: np.ndarray, reflectance: np.ndarray) -> tuple[np.ndarray, ...]:
    ...
    for i, spectrum in enumerate(reflectance):
        try:
            BIC[i] = my_argmin(wavelength, spectrum, x0=pos_min_1, dx=100., n=3)
        except:
            BIC[i] = np.nan
        try:
            BIIC[i] = my_argmin(wavelength, spectrum, x0=pos_min_2, dx=200., n=3)
        except:
            BIIC[i] = np.nan

        try:
            wvl_max_1 = my_argmax(wavelength, spectrum, x0=pos_max_1, dx=200., n=3)
        except:
            wvl_max_1 = np.nan

        try:
            wvl_max_2 = my_argmax(wavelength, spectrum, x0=pos_max_2, dx=300., n=3)
        except:
            wvl_max_2 = np.nan

        try:
            wvl_max_3 = my_argmax(wavelength, spectrum, x0=pos_max_3, dx=200., n=3)
        except:
            wvl_max_3 = np.nan
        """
        ...
        wvl_start = np.where(wavelength == np.round(wvl_max_1 / dx) * int(dx))[0][0]
        wvl_stop = np.where(wavelength == np.round(wvl_max_2 / dx) * int(dx))[0][0]
        ...
        wvl_start = np.where(wavelength == np.round(wvl_max_2 / dx) * int(dx))[0][0]
        wvl_stop = np.where(wavelength == np.round(wvl_max_3 / dx) * int(dx))[0][0]
