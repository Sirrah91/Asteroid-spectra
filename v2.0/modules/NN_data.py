...
from modules.NN_config_taxonomy import classes, num_labels  # num_labels is not used by the compositional
...
def load_compositional_data(filename_data: str, clean_dataset: bool = True, keep_all_labels: bool = False,
...
def load_taxonomical_data(filename_data: str, clean_dataset: bool = True, return_meta: bool = False,
...
def split_compositional_data_proportional(x_train: np.ndarray, y_train: np.ndarray,
...
def split_taxonomical_data_proportional(x_train: np.ndarray, y_train: np.ndarray,
                                        rnd_seed: int = 42) -> tuple[np.ndarray, ...]:
...
def split_meta_proportional(meta: np.ndarray, y_data: np.ndarray,
                            model_type: Literal["compositional", "taxonomical"]) -> tuple[np.ndarray, ...]:

    # Might be inaccurate if you use different seed for splitting data and metadata

    if model_type == "compositional":
        split_data_proportional = split_compositional_data_proportional
    else:
        split_data_proportional = split_taxonomical_data_proportional
