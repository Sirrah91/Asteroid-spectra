...
def evaluate(model_names: list[str], filename_or_data: str | np.ndarray,
             subfolder_model: str = "composition") -> np.ndarray:
    ...
    # loading needed values
    if "taxonomy" in subfolder_model:
    ...
    predictions = average_and_normalise(predictions, "taxonomy" not in subfolder_model)
    print("-----------------------------------------------------")
...
def evaluate_test_data(model_names: list[str], x_test: np.ndarray, y_test: np.ndarray,
                       ...
                       subfolder_model: str = "composition") -> tuple[np.ndarray, ...]:
    if not model_names:
        raise ValueError('"model_names" is empty')
    ...
    elif "composition" in subfolder_model or "accuracy_test" in subfolder_model:
    ...
    from modules.NN_config_taxonomy import metrics  # needed to distinguish between taxonomy and composition
    ...
    for idx, model_name in enumerate(model_names):
        ...
        if idx == 0:
            taxonomy = np.any([metrics[0].__name__ in metric.name for metric in model.metrics])

    # Trimmed means and normalisations to 1
    predictions = average_and_normalise(predictions, not taxonomy)
    if do_train:
        predictions_train = average_and_normalise(predictions_train, not taxonomy)
    if do_val:
        predictions_val = average_and_normalise(predictions_val, not taxonomy)
    ...
    if show_result_plot:
        print("Result plots")
        _result_plots(y_test, predictions, taxonomy)

    if show_control_plot:
        print("Control plots")
        if do_val:
            _control_plots(y_val, predictions_val, taxonomy)
        if do_train:
            _control_plots(y_train, predictions_train, taxonomy)
    ...
    if do_train:
        _print_info(metric, y_train, predictions_train, taxonomy, "train")
    if do_val:
        _print_info(metric, y_val, predictions_val, taxonomy, "validation")
    acc = _print_info(metric, y_test, predictions, taxonomy, "test")
...
def _result_plots(y_true: np.ndarray, y_pred: np.ndarray, taxonomy: bool) -> None:
    quiet = True
...
def _control_plots(y_true: np.ndarray, y_pred: np.ndarray, taxonomy: bool) -> None:
    quiet = True
...
def _print_info(metric: Callable, y_true: np.ndarray, y_pred: np.ndarray, taxonomy: bool, which: str) -> np.ndarray:
    if taxonomy:
