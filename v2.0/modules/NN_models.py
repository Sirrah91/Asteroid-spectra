...
from modules.NN_losses_metrics_activations import output_activation, loss, loss_tuner
from modules.NN_losses_metrics_activations import mse, rmse, Lp_norm, mae, quantile, r2, sam, sse

from modules.NN_config import num_minerals, endmembers_counts, num_labels

# choose metrics
# metrics = [mse, rmse, Lp_norm, mae, quantile, r2, sam, sse]
metrics = [mse]
...
def MLP_model(input_shape: tuple[int, ...], params: dict[str, str | int | float | bool | list[int]]) -> Functional:
    ...
    outputs = Activation(output_activation)(dense)
...
class MyHyperModel(HyperModel):
    ...
    def common_hp(self, hp):
        ...
        if num_minerals > 0 and np.sum(endmembers_counts) > 0:
            self.alpha = hp.Float("alpha", min_value=min(self.params["alpha"]), max_value=max(self.params["alpha"]),
                                  sampling="log")
    ...
    def build(self, hp):
        ...
        if num_minerals > 1:
            loss_used = loss_tuner(self.alpha)
        else:
            loss_used = loss
