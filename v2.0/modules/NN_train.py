...
def _print_info(model: Functional, x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
                model_usage: Literal["composition", "taxonomy"]) -> None:
...
def gimme_model_name(model_usage: Literal["composition", "taxonomy"]) -> tuple[str, str]:
    if model_usage == "taxonomy":
...
def hp_tuner(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
             composition_or_taxonomy: Literal["composition", "taxonomy"]) -> list[str]:

    # loading needed values
    if composition_or_taxonomy == "composition":
    ...
    elif composition_or_taxonomy == "taxonomy":
