...
# ------------------------------------------------------------------------------------------------------------------------
what_to_run = "both"  # can be either "indices", "averaging", or "both"

what_asteroid = "Eros"
spectra_dir = "".join((_path_data, "/asteroids/", what_asteroid, "/"))

# longitude and latitude resolution
dlon, dlat = 1., 1.
dlon_half, dlat_half = dlon / 2., dlat / 2.

lon_grid, lat_grid = safe_arange(0., 360., dlon, endpoint=True), safe_arange(0., 180., dlat, endpoint=True)

# interpolation method
# linear is much faster; is cubic interpolation good enough if there are wider gaps in spectra?
...
if "Itokawa" == what_asteroid:
    ...
    wavelengths_new = safe_arange(820., 2360., 20., endpoint=True)  # nm
    normalised_at_wvl = 1500.  # nm
...
def read_data(asteroid_name: str, file_dir: str) -> np.ndarray:
    ...
    with h5py.File("".join((file_dir, "/", asteroid_name, "_data.h5")), "r") as f:
        obs_data = np.array(f[data_name])
...
def read_indices(asteroid_name: str, file_dir: str) -> list[str]:
    ...
    with open("".join((file_dir, "/", asteroid_name, "_indices.txt")), "r") as text_file:
...
def save_indices(asteroid_name: str, file_dir: str, indices_to_save: np.ndarray) -> None:
    with open("".join((file_dir, "/", asteroid_name, "_indices.txt")), "w") as f:
        for i in indices_to_save:
            f.write("".join((str(list(i)), "\n")))
...
def saving_spectra(asteroid_name: str, file_dir: str, filtered_pt_coords: np.ndarray, weighted_spectra: np.ndarray,
    ...
    # remove NaNs (if polygons do not overlap)
    ...
    filtered_pt_coords[:, 1] -= 90.  # co-latitude to latitude

    # Creates the h5-file and saves the data as a new dataset
    with h5py.File("".join((file_dir, "/", asteroid_name, "_averaged.h5")), "w") as f:
        f.create_dataset("spectra", data=weighted_spectra)
        f.create_dataset("coordinates", data=filtered_pt_coords)
        f.create_dataset("wavelengths", data=wavelengths)


# ------------------------------------------------------------------------------------------------------------------------
def convert_lat_to_colat(asteroid_name: str, coord_vector: np.ndarray) -> tuple[float, ...]:
    if asteroid_name == "Itokawa":
        # 1st corner y-coordinate etc.
        y1, y2, y3, y4 = coord_vector[[0, 6, 4, 2]] + 90.

        return y1, y2, y3, y4
    else:
        # lat min, max
        y1, y2 = coord_vector[[0, 1]] + 90.
...
def preprocess_lon(asteroid_name: str, coord_vector: np.ndarray) -> tuple[float, ...]:
    ...
    if asteroid_name == "Itokawa":
        for k in [3, 5, 7]:
            if coord_vector[k] - coord_vector[1] >= 180.:
                coord_vector[k] -= 360.
            if coord_vector[k] - coord_vector[1] <= -180.:
                coord_vector[k] += 360.
        ...
        if coord_vector[3] - coord_vector[2] >= 180.:
            coord_vector[3] -= 360.
        if coord_vector[3] - coord_vector[2] <= -180.:
            coord_vector[3] += 360.
...
def convert_vector_lat_to_colat(asteroid_name: str, coord_vector: np.ndarray) -> tuple[np.ndarray, ...]:
    if asteroid_name == "Itokawa":
        # 1st corner y-coordinate etc.
        y1, y2, y3, y4 = np.transpose(coord_vector[: [0, 6, 4, 2]]) + 90.

        return y1, y2, y3, y4
    else:
        # lat min, max
        y1, y2 = np.transpose(coord_vector[:, [0, 1]]) + 90.

        return y1, y2, y2, y1


def preprocess_vector_lon(asteroid_name: str, coord_vector: np.ndarray) -> tuple[np.ndarray, ...]:
    # It is not possible for measurement to be over 180 degrees
    if asteroid_name == "Itokawa":
        for k in [3, 5, 7]:
...
def coordinates_to_polygons(asteroid_name: str, coord_vector: np.ndarray) -> np.ndarray:
    y1, y2, y3, y4 = convert_vector_lat_to_colat(asteroid_name, coord_vector)
    x1, x2, x3, x4 = preprocess_vector_lon(asteroid_name, coord_vector)
...
def polygon_within_intersect_polygon(polygon1, polygon2) -> int:
    # conditions were set to minimise processing time
    # no overlap
    if polygon1.distance(polygon2) > 0.:
        return 0
    # intersect or touch
    # removing touches is painfully slow; it is efficiently removed by multiplying with intersect area in
    # the calc_weights; it can raise warnings "division by zero" there
    elif polygon1.intersects(polygon2):  # and not polygon1.touches(polygon2)
        return 2
    # fully overlap
    else:  # polygon1.contains(polygon2) or polygon2.contains(polygon1):
        return 1
...
def figure_layout(pt_or_polygon, polygon) -> int:
...
def get_indices_v1(asteroid_name: str, obs_data: np.ndarray, used_indices: np.ndarray,
    ...
    print("\n[INFO] Testing how many polygons overlap given {patch}".format(patch=polygon_or_point))
    ...
    for lat in tqdm(lat_grid):
        for lon in lon_grid:
            ...
            for index in used_indices:
                ...
                y1, y2, y3, y4 = convert_lat_to_colat(asteroid_name, vector_lon_lat)
                x1, x2, x3, x4 = preprocess_lon(asteroid_name, vector_lon_lat)

                coordinates = np.array([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
                polygon_data = Polygon(coordinates).convex_hull

                # this is slow but precise
                if figure_layout(patch, polygon_data) > 0:
                    row.append(index)
    ...
    for lat in tqdm(lat_grid):
        for lon in lon_grid:
            ...
            for poly_data, index in zip(polygons_data, used_indices):
                ...
                if figure_layout(patch, poly_data) > 0:
                    row.append(index)
...
def import_data(asteroid_name: str, file_dir: str) -> tuple[np.ndarray, ...]:
    ...
    df = np.array([json.loads(line) for line in lines[:-1]], dtype=object)  # last line is empty
...
def calc_areas(asteroid_name: str, polygon_data_coords: np.ndarray, polygon_patch_coords: np.ndarray) -> np.ndarray:
    ...
    for i, (data_coords, patch_coords) in enumerate(zip(polygon_data_coords, polygon_patch_coords)):
        ...
        for j, coords in enumerate(data_coords):
            ...
            y1, y2, y3, y4 = convert_lat_to_colat(asteroid_name, coords)
            x1, x2, x3, x4 = preprocess_lon(asteroid_name, coords)

            coordinates = np.array([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
            polygon_data = Polygon(coordinates).convex_hull
            data_area = polygon_data.area

            patch_polygon_layout = figure_layout(patch, polygon_data)

            if patch_polygon_layout == 1:  # fully overlap
                area[j] = 1. / data_area
            elif patch_polygon_layout == 2:  # intersection
...
def weighting(filtered_spectra: np.ndarray, weights: np.ndarray) -> np.ndarray:
    return np.array([np.sum(np.transpose(np.transpose(filtered_spectrum) * weight / np.sum(weight)), axis=0)
