...
from modules.utilities_spectra import denoise_and_norm, save_data, combine_files, used_indices
from modules.utilities import flatten_list, stack, my_mv, check_dir, safe_arange, normalise_in_rows

from modules.NN_config import mineral_names_short, endmember_names, minerals_used, endmembers_used
...
def resave_Itokawa_Eros() -> None:
    ...
    data = denoise_and_norm(data=data, wavelength=wavelengths, denoising=denoise,
                            normalising=normalise, normalised_at_wvl=1500, sigma_nm=denoising_kernel_width)
...
def resave_data_for_shapeViewer(y_pred: np.ndarray, asteroid_name: str,
                                taxonomical_or_compositional: Literal["compositional", "taxonomical"],
                                indices_to_save: list[int] | None = None, add_blank_data: bool = True) -> None:
    if indices_to_save is None:
        indices_to_save = range(np.shape(y_pred)[1])

    num_digits = 3

    if taxonomical_or_compositional == "compositional":
        quantity_names = flatten_list([mineral_names_short] + endmember_names)[used_indices(minerals_used,
                                                                                            endmembers_used)]
    elif taxonomical_or_compositional == "taxonomical":
        quantity_names = list(classes.keys())
    else:
        raise ValueError('"taxonomical_or_compositional" must be either "taxonomical" or "compositional"')
    ...
    angle = np.ones(np.shape(lon)) * np.sqrt(dlon * dlat)  # angle size of our data
    ...
    for index_to_save in indices_to_save:
        ...
        if add_blank_data:  # to make the shpeViewer colorbar be from 0 to 100
            # these points are not visible
            tmp = np.array([["g", "-89.9", "0.0", "0.1", "0.0"],
                           ["g", "-89.9", "1.0", "0.1", "100.0"]])
            results = stack((tmp, results), axis=0)

        filename = "".join((_path_data, "/shapeViewer/", asteroid_name, "_", quantity_name, ".dat"))
        check_dir(filename)
        np.savetxt(filename, results, fmt="%s", delimiter=" ")
