...
outdir_composition = "".join((outdir, "/composition/"))
check_dir(outdir_composition)

outdir_taxonomy = "".join((outdir, "/taxonomy/"))
...
def plot_scatter_plots(y_true: np.ndarray, y_pred: np.ndarray, used_minerals: np.ndarray, used_endmembers: np.ndarray,
    ...
    if number_of_minerals > 1:
        ...
        for i, axis in enumerate(ax):
            ...
            axis.errorbar(x_tmp[:, i], y_tmp[:, i], yerr=error_pred[i], xerr=error_true[i], fmt=eb, ls="", zorder=1)

            axis.set_xlabel("Actual (vol\%)")
            axis.tick_params(axis="both")
            axis.axis("square")
            axis.set_title(mineral_titles[i].title())
    ...
    for i in range(len(used_endmembers)):
        ...
        for j, axis in enumerate(ax):
            ...
            axis.errorbar(x_tmp[:, j], y_tmp[:, j], yerr=error_pred[j], xerr=error_true[j], fmt=eb, ls="", zorder=1)
...
def plot_model_history(model: Functional, quiet: bool = False) -> None:
    ...
    if model.metrics_names[1] == "mse":  # MSE to RMSE
        ...
        labely = model.metrics_names[1].title()
...
def plot_corr_matrix(labels: np.ndarray, corr_matrix: pd.DataFrame, suf: str, quiet: bool = False) -> None:
    ...
    color[np.abs(corr_matrix) < 0.25] = "k"

    for ix in range(len(labels)):
        for iy in range(len(labels)):
            ax.text(iy, ix, "".join("{:.2f}".format(corr_matrix[ix, iy])),
                    ha="center", va="center", color=color[ix, iy], fontsize=SMALL_SIZE)
...
def plot_acc_test_results(filename: Literal["composition", "taxonomy"]) -> None:
