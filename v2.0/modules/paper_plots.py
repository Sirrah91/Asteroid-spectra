...
def plot_Fa_vs_Fs(y_true: np.ndarray, y_pred: np.ndarray) -> None:
    ...
    meta_train, meta_val, meta_test = split_meta_proportional(meta, y_data, "composition")
    types = meta_test[:, 7]
...
def plot_scatter_NN_BC() -> None:
    ...
    y_pred, accuracy = evaluate_test_data(model_names, x_test, y_test, subfolder_model="composition")
...
def plot_spectra_2() -> None:
    ...
    data = np.load(filename, allow_pickle=True)  # to read the file
    types = np.array(data["labels"], dtype=str).ravel()  # "metadata" cannot be here because it contains original types
    ...
    for k, unique_type in enumerate(np.unique(types)):
        ...
        spectra = normalise_in_rows(spectra, np.transpose(spectra[:, wavelengths == norm_at]))  # normalise
...
def plot_surface_spectra_shapeViewer(asteroid_name: str, what_prediction: str | list[str] = "all") -> None:
    ...
    rows, columns = best_blk(len(order))
    ...
    image_type = "min_max_cbar"  # Either "min_max_cbar" or "0-100_cbar"
    path_to_images = "".join((_path_asteroid_images, "/shapeViewer/", image_type, "/"))
    path_to_background = "".join((_path_asteroid_images, "/shapeViewer/"))

    filenames = ["".join((asteroid_name, "_", o, ".png")) for o in order]

    for which in what_prediction:

        reversed_caxis = which in ["Q", "olivine"]  # these two have reversed coloraxis

        if which in classes.keys():  # taxonomy map
        ...
        # to adjust colorbar
        if reversed_caxis:
            fname = "".join((_path_data, "/shapeViewer/", image_type, "/", asteroid_name, "_", which, "_reversed.dat"))
            y_pred = 100. - np.array(np.loadtxt(fname, dtype=object)[:, -1], dtype=float)
            cmap += "_r"
        else:
            fname = "".join((_path_data, "/shapeViewer/", image_type, "/", asteroid_name, "_", which, ".dat"))
            y_pred = np.array(np.loadtxt(fname, dtype=object)[:, -1], dtype=float)


        fig, ax = plt.subplots(rows, columns, figsize=(12, 6), width_ratios=[2, 1])
        ax = ax.ravel()

        for i, axis in enumerate(ax):
            fname = filenames[i].replace("_", "".join(("_", which, "_")))
            fname = "".join((path_to_images, fname))
            ...
            im_frame_bg = cv2.imread("".join((path_to_background, filenames[i])))[:, :, ::-1]  # background BGR to RGB
            ...
            if asteroid_name == "Itokawa":
                ...
                if i == 0:
                    axis.text(0.8, 0.7, "1", **text_kwargs)
            ...
            fig_name = "".join((asteroid_name, "_", which, "_SV.", big_fig_format))
            fig.savefig("".join((outdir_surfaces, "/", fig_name)), format=big_fig_format, **savefig_kwargs, **pil_kwargs)
            plt.close(fig)
...
def plot_EI_type_hist(y_Eros: np.ndarray, y_Itokawa: np.ndarray, tax_type: str) -> None:
    nbins = safe_arange(0., 100., 5., endpoint=True)

    xticks = safe_arange(0., 100., 10., endpoint=True)
    left, right = 0.0, 100.

    inds = classes[tax_type]
    ...
    for i, data in enumerate(ei_list):
        ...
        ax[i].set_xlabel("".join((tax_type,"-type match score (\%)")))
        if i == 0:
    ...
    fig_name = "".join(("Eros_Itokawa_", tax_type, "_hist", ".", fig_format))
    fig.savefig("".join((outdir_taxonomy, "/", fig_name)), format=fig_format, **savefig_kwargs, **pil_kwargs)
    plt.close("all")
...
def plot_ast_type_histogram(y_true: np.ndarray, y_pred: np.ndarray, type1: str, type2: str) -> None:
    ...
    ax[0].set_xlabel("".join(("Match score to ", type1, " (\%)")))
    ax[0].set_ylabel("Counts")
    ax[0].legend()
    ...
    ax[1].set_xlabel("".join(("Match score to ", type2, " (\%)")))
...
def plot_Eros_spectra():
    titles = ["NIS", "DeMeo et al. (2009)", "NIS corrected"]

    ls = ["-k", "-r", "-b"]

    # NIS raw
    Eros_dir = "/asteroids/Eros/"
    NIS = np.load("".join((_path_data, Eros_dir, "Eros_NIS-denoised-norm.npz")), allow_pickle=True)
    NIS, wavelengths = NIS["spectra"], NIS["wavelengths"]

    # DeMeo et al. (2009)
    DM = np.load("".join((_path_data, "asteroid_spectra-denoised-norm.npz")), allow_pickle=True)
    eros = "433" == DM["metadata"][:, 0]
    DM, DM_wvl = DM["spectra"][eros], DM["wavelengths"]
    fun = interp1d(DM_wvl, DM, kind="cubic")
    DM = fun(wavelengths) / fun(1300.)

    # NIS corrected
    NIS_corr = np.load("".join((_path_data, "Eros-denoised-norm.npz")), allow_pickle=True)
    NIS_corr = NIS_corr["spectra"]
    ...
    fig_name = "".join(("Eros_correction", ".", big_fig_format))
    fig.savefig("".join((outdir, "/", fig_name)), format=big_fig_format, **savefig_kwargs, **pil_kwargs)
    plt.close(fig)
