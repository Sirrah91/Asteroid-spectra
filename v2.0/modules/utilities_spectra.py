...
from modules.utilities import check_dir, flatten_list, normalise_in_rows, stack
...
def save_data(final_name: str, spectra: np.ndarray, wavelengths: np.ndarray, metadata: np.ndarray,
              ...
              metadata_key: np.ndarray | None = None, other_info: dict | None = None, subfolder: str = "") -> str:
...
def npz_to_dat(filename: str, subfolder: str = "") -> None:
    npz_name = "".join((_path_data, "/",subfolder, "/", filename))
    dat_name = npz_name.replace(".npz", "")

    data = np.load(npz_name, allow_pickle=True)

    for file in data.files:
        fmt = "%.5f" if np.issubdtype(np.result_type(data[file]), np.number) else "%s"
        np.savetxt("".join((dat_name, "-", file, ".dat")), data[file], fmt=fmt, delimiter='\t')
...
def filename_adjustment(filename: str, denoising=denoise, normalising=normalise) -> str:
    tmp = Path(filename)
...
def denoise_and_norm_file(file: str, denoising: bool, normalising: bool, sigma_nm: float = 7.,
    ...
    # load the data
    data_file = "".join((path_to_data, "/", file))
    ...
    final_name = filename_adjustment(data_file, denoising=denoising, normalising=normalising)

    save_data(final_name, spectra=spectra, wavelengths=xq, labels=data["labels"],
...
def combine_composition_and_taxonomy_predictions(filename: str, comp_models: list[str] | None = None,
    ...
    if comp_models is None:
        ...
        comp_models = collect_all_models(suffix="".join(("CNN_", asteroid_name)), subfolder_model="composition",
                                         full_path=False)

    if tax_models is None:
        asteroid_name = filename[:filename.find("-")]
        tax_models = collect_all_models(suffix="".join(("CNN_", asteroid_name)), subfolder_model="taxonomy",
                                        full_path=False)

    pred_comp = evaluate(comp_models, filename, subfolder_model="composition")
    pred_tax = evaluate(tax_models, filename, subfolder_model="taxonomy")
