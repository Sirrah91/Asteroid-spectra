...
def check_used_quantities(used_minerals: np.ndarray, used_endmembers: list[list[bool]],
    ...
    # no mineral for endmember (one allowed if no minerals)
...
def bin_to_used(bin_code: str, separator: str | None = None,
    ...
    # this function converts info from binary code in a name of a model into mineral abundances and composition
    ...
    if len(used_minerals) != len(used_endmembers):
        raise ValueError(f'The length of the used minerals does not equal to the length of the used end-members.\n'
                         f'Probably incorrect separator. Bin code "{bin_code}", separator "{separator}".')
