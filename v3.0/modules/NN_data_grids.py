...
def check_grid(grid: np.ndarray) -> None:
    if grid[1] < grid[0]:
    ...
    if  not (isclose(grid_resolution_fraction, 0.0, abs_tol=_num_eps) or
             isclose(grid_resolution_fraction, 1.0, abs_tol=_num_eps)):
