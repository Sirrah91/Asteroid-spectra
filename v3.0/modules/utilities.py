...
def return_mean_std(array: np.ndarray, axis: int | None = None) -> tuple[np.ndarray, np.ndarray]:
    ...
    # if there is less than 2 numeric (finite) samples
...
def my_argextreme(min_or_max: Literal["min", "max"], x: np.ndarray, y: np.ndarray,
    ...
    # return extreme of parabola if it is not far (rtol pixel) from the local extreme
    ...
    if try_numpy:
        # if it is too far from local maximum, use standard numpy fit and try it again
        params = my_polyfit(x_fit, y_fit, 2, method="numpy")
        extreme = -params[1] / (2. * params[0]) * x_std + x_mean
...
def normalise_array(array: np.ndarray,
    ...
    # to force correct dimensions (e.g. when passing output of interp1d)
...
def denoise_array(array: np.ndarray, sigma_px: float, remove_mean: bool = True) -> np.ndarray:
    ...
    *_, cols = np.shape(array)
    ...
    if remove_mean:  # here I assume that the noise has zero mean
