...
def filename_adjustment(filename: str, denoising: bool, normalising: bool) -> str:
    tmp = Path(filename)
    final_name = tmp.name.replace(tmp.suffix, "")  # remove suffix

    if denoising:
        if "_denoised" not in final_name:
            final_name += f"{_sep_out}denoised"
    if normalising:
        if "_norm" not in final_name:
...
def if_no_test_data(x_train: np.ndarray | None, y_train: np.ndarray,
    ...
    if not is_empty(y_val):  # If test portion is zero then use validation data
...
def is_taxonomical(model: str | Functional | None = None, bin_code: str | None = None) -> bool:
    ...
    if isinstance(model, Functional):  # model was compiled when loaded
        ...
        # last layer is output activation or dense (if activation is set as parameter of Dense)
...
def error_estimation_overall(y_true: np.ndarray, y_pred: np.ndarray, actual_error: np.ndarray | float = 3.,
    ...
    RMSE = my_rmse(used_minerals=used_minerals, used_endmembers=used_endmembers,
                   cleaning=True, all_to_one=False)(y_true, y_pred).numpy() / 100.  # is multiplied with 100 in the code
